...
let cachedSchema: SchemaDefinition<GenericSchema, boolean> | null = null;

export type NoruSchema = SchemaDefinition<GenericSchema, boolean>;

export async function getSchema(): Promise<NoruSchema> {
	if (cachedSchema) {
		return cachedSchema;
	}

	const { authTables } = await import("@convex-dev/auth/server");

	cachedSchema = defineSchema({
		...authTables,
